...
# Lecture 11: File system implementations

## Implementation overview

* General disk structure
    * a 1-D array of **logical blocks**
    * Logical block: smallest accessible unit (512B - 4KB)
    * Logical blocks are mapped to **disk sectors**
* Disk organization
    * Master boot record (MBR)
        * at sector 0
        * include simple boot code and partition table
    * Partitions
        * each partition can contain an independent *file system*
    * A *file system* contains
        * OS boot-up information (???)
        * Partition details (e.g. # of blocks, # and location of free blocks)
        * Directory structure
        * Files information
        * Actual file data
    * ![](https://i.imgur.com/Q2EFGBc.png)

## File implementation

File = a collection of logical blocks

* Internal fragmentation
    * the last block may contain wasted space
* Criteria for a good file implementation
    * Keep track of logical blocks
    * Allow efficient access
    * Disk space is utilized efficiently

### Contiguous

* Pros
    * simple to keep track of (only need starting block number + length)
    * fast access (only need to seek the starting block, then sequential access)
* Cons
...
* General idea
    * each disk block stores actual file data + **next disk block number**
    * file information stores: first and last disk block number
        * stores the last disk block number so that *appending* is faster
* Pros
    * solves external fragmentation
* Cons
    * random access may be very slow
    * part of the block is used to store the next disk block number (overhead)
    * less reliable (one broken block breaks the whole linked list)

### Linked list 2.0 - FAT (file allocation table)

* General idea
    * move all block pointers into a single table
    * can be cached in memory for faster traversal on the linked list
    * **FAT is in memory all the time**
* Pros
  * faster random access (traversing linked list happens in RAM instead of disk)
* Cons
  * FAT keeps track of all disk blocks in a partition
    * can be huge when disk is huge
    * consume valuable memory (RAM) space

### Indexed allocation

* General idea
  * each file has an index block
  * in which stores the indices of all blocks belonging to this file
* Pros
  * less memory overhead (only need to load one index block for one open file)
  * fast direct access
* Cons
  * **limited** maximum file size
    * max # of blocks = # of index block entries = index block size / entry size
  * index block overhead
* Variations
  * Linked scheme
    * keep a linked list of index blocks (i.e. each index block contains a pointer to the next index block)
  * Multi-level index
    * first-level index block points to several second-level index blocks
    * can have many levels
  * Combined scheme
    * A combination of efficiency (for small file) and flexibility (still allow large file)
    * e.g. Ext2 I-node (more on that in the next lecture)

## Free space management

* Free space management
  * maintains free blocks information
  * allocate
    * when a file is created or appended
    * remove free block from the list
  * free
    * when a file is deleted or truncated
    * add a free block to the lisk
...
* Pros
  * provide a good set of manipulations (e.g. first free block, n-consecutive free blocks, by bit level operation)
* Cons
  * needs to be in memory for efficiency
...
* General idea
  * ???
* Pros
  * easy to allocate free block
  * only the first pointer is needed in memory
    * others can be cached for better efficiency
* Cons
  * high overhead
    * can be mitigated by storing free block list in free blocks (similar to indexed allocation)

## Directory implementation

* Directory structure's role
  * keep track of files in a directory
    * possibly with file metadata (not mandatory)
  * map file names to file information (filename, metadata, disk block information)
    * store everything in the directory entry
      * a simple scheme: fixed size entry
    * store only the file name and points to some other data structure for file information
* Path name
  * List of directory names traversed from root
* Sub-directory is usually stored as a **file entry** with special type in a directory
...
* File entry
  * store file name (and possibly file metadata)
  * store file information **OR** pointer to file information
* Locate a file in a directory
  * linear search, inefficient for large directories or deep traversal
  * can use cache to remember the latest few searches as user usually move up/down a path

### Hash table

Each directory has a hash table of size `N`, each file name is hashed into an index from 0 to `K-1`, `HashTable[I]` is inspected to match the file name. Usually *chained collision resolution* is used.

* Pros
  * fast lookup
* Cons
  * hash table has limited size (small hash table = more collision)
  * depends on a good hash function (fast and effective)

## Disk IO scheduling

* Magnetic disk movement
  * *Seek*: change track
  * *Rotation*: change sector
* I/O scheduling
  * to reduce **overall waiting time**
  * focus on reducing **seeking time** as rotation latency is hard to mitigate
  * balance 1. high throughput and 2. fairness among processes
* I/O scheduling algorithms
  * numbers = track number
  * FCFS
  * SSF (shortest seek first)
  * SCAN (aka *Elevator*)
    * SCAN (bi-directional, innermost <-> outermost)
    * C-SCAN (uni-directional, outermost <-> innermost)
  * Deadline (3 queues)
    * sorted
    * read FIFO
    * write FIFO
  * noop
  * cfq (completely fair queuing) - time slice and per-process sorted queues
  * bfq (budget fair queuing) - fair sharing based on # of sectors requested
...
* Layout
  * ![](https://i.imgur.com/bm4nP2Z.png)
* FAT (file allocation table)
  * one entry per data block/cluster
  * content: 
    * free? - `FREE` code
    * occupied (and next block)? 
      * block number of the next block **OR**
      * `EOF` code (i.e. `NULL` pointer)
    * damaged? - `BAD code
  * OS caches FAT in memory for fast linked list traversal
  * FAT16 has a FAT of $2^16$ entries, each entry is 16 bits
    * total addressable blocks are fewer than $2^16$ because codes need encoding space
* Directory and file information
  * directory is stored as a **special file**
  * **root directory** is stored in a special location
    * other directories are stored in data blocks
  * sub-directory or files are represented as a **directory entry**
  * Directory entry
    * fixed-size, 32 bits per entry
    * contains:
      * name + extension
        * limited to 8+3 characters
        * first byte may have special meaning (Deleted, End of directory entries, Parent directory, etc.)
      * attributes (read-only? direct/file flag, hidden?)
      * creation date + time
        * Year is limited to 1980 to 2107
        * Accuracy of second is ±2 seconds
      * first disk block + file size
        * FAT12,16,32 have different bytes for first disk block
    * ![](https://i.imgur.com/QNhX3Ei.png)
      * this is an example of FAT16 because "first disk block" is 2 bytes (16 bits)
* File deletion
  * "Delete" the directory entry:
    * Set first letter in filename to a special value 0xE5
  * Free data blocks:
    * Set FAT entries in linked list to `FREE`
      * the entire list will be set to `FREE` (i.e. will traverse through the linked list)
    * Actual data blocks **remain intact**
      * Can attempt to undelete
        * can recover part of the filename
        * can recover the first block of file
  * **No** free space management
    * free space can only be calculated by going through the FAT
* Larger single partition size support
  * Disk cluster
    * use a number of contiguous physical disk blocks as the smallest allocation unit (logical block)
    * Pros: larger usable partition
    * Cons: larger **internal fragmentation**
  * Increase FAT size
    * Example (4KiB cluster size)
      * ![](https://i.imgur.com/jdEdOY6.png)
      * actual size is lesser because special values (`EOF`, `FREE`, etc) reduces total number of valid data block/cluster indices
* Long filename support
  * VFAT (Virtual FAT)
    * support filenames up to 255 characters
    * A work around rather than redesign
      ...
      * ![](https://i.imgur.com/nwar71e.png)

## Extended-2

### Overview

![](https://i.imgur.com/PBpUZnM.png)
...
* Group Descriptors
  * Describe **each** of the block group (i.e. all block groups)
  * Includes:
...
An I-node is 128 bytes.

* Structure
  * ![](https://i.imgur.com/Yy3lYau.png)
* Includes
  * file metadata
  * data block addresses
* Data block structure
  * ![](https://i.imgur.com/eU8ipjZ.png)
  * 15 data block pointers
    * 12 direct pointers
    * 13th single indirect block
    * 14th double indirect block
    * 15th triple indirect block
  * fast access to small file (direct pointers)
  * flexibility in handling huge file (indirect blocks)
* Directory structure
  * data blocks of a directory stores
    * a linked list of directory entries for file/sub-directories information within this directory
  * each directory entry stores
    * I-node number for that file/sub-directory
    * size of the entry (i.e. a pointer to the next entry)
    * length of the file/directory name
    * file/directory name (up to 255 bytes)
  * ![](https://i.imgur.com/DGT92wf.png)
...
* Deleting a file:
  ...
  * Since no real block is wiped, it's still possible to undelete a file in ext2.
* Hard link: (original file `X` with I-node index `XN`, want to create a link in `B`)
  * Creates a directory entry in `B`
    * which use the same I-Node number `XN`
    * Can have different filename
  * I-node maintains a reference count
    * decremented for every deletion, when decreased to 0, delete the file represented by this I-node
  * ![](https://i.imgur.com/6gqRD1p.png)
* Symbolic Link:
  ...
  * As only the pathname is stored, the link can be easily invalidated:
    ...
    * Involve a search to locate actual I-Node number of target file, just like a normal file opening operation
  * ![](https://i.imgur.com/vVpl7nq.png)
